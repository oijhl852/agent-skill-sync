...
仓库现在只提供一个可安装目录：`seedance-storyboard`。安装一次，即可获得主编排、13 个按需读取的子 skill 模块、4776 条社区提示词语料、完整剧本案例和辅助脚本。
...
Seedance Storyboard 把这些风险拆成独立步骤，并让主 skill 在每个阶段读取对应规则。可以把它理解成剪辑软件的主程序加一组内置模块：平时只打开需要的工具，但整套流程始终由同一个项目管理。
...
主入口 [seedance-storyboard/SKILL.md](seedance-storyboard/SKILL.md) 负责判断任务类型、安排执行顺序和传递上一步结果。每个子模块只负责一件事，主 skill 在执行前读取对应的 `SKILL.md`，不会一次性加载全部资料。
...
| 阶段 | 子 skill | 职责 |
|---|---|---|
| 内容理解 | [sd-read-script](seedance-storyboard/references/subskills/sd-read-script/SKILL.md) | 剧本读取、分场和保真表 |
| 内容理解 | [sd-story-adapt](seedance-storyboard/references/subskills/sd-story-adapt/SKILL.md) | 故事改编为剧本 |
| 片段规划 | [sd-segment-split](seedance-storyboard/references/subskills/sd-segment-split/SKILL.md) | 4-15 秒拆段和戏剧节拍 |
| 分镜设计 | [sd-dialogue](seedance-storyboard/references/subskills/sd-dialogue/SKILL.md) | 对话镜头和反应镜头 |
| 分镜设计 | [sd-action](seedance-storyboard/references/subskills/sd-action/SKILL.md) | 动作、打斗和蒙太奇 |
| 资产准备 | [sd-asset-guide](seedance-storyboard/references/subskills/sd-asset-guide/SKILL.md) | 角色、场景和道具资产清单 |
| 提示词 | [sd-prompt](seedance-storyboard/references/subskills/sd-prompt/SKILL.md) | 结构化和时间轴提示词 |
| 提示词 | [sd-prompt-library](seedance-storyboard/references/subskills/sd-prompt-library/SKILL.md) | 精选案例和提示词框架 |
| 提示词 | [sd-community](seedance-storyboard/references/subskills/sd-community/SKILL.md) | 4776 条本地社区语料和评分 |
| 质量 | [sd-quality](seedance-storyboard/references/subskills/sd-quality/SKILL.md) | 保真、密度、资产和衔接检查 |
| 面板 | [sd-panel](seedance-storyboard/references/subskills/sd-panel/SKILL.md) | 浏览器面板连接和空白片段建立 |
| 面板 | [sd-chip](seedance-storyboard/references/subskills/sd-chip/SKILL.md) | 资产名称、真实 ID 和 chip 对应 |
| 面板 | [sd-inject](seedance-storyboard/references/subskills/sd-inject/SKILL.md) | API 注入、刷新验证和失败停止 |

## 社区提示词语料

完整语料位于 [community-prompts-4776.csv](seedance-storyboard/references/subskills/sd-community/corpus/community-prompts-4776.csv)，包含标题、描述、完整提示词、来源和作者字段。
...
- [司马光砸缸](seedance-storyboard/references/examples/司马光砸缸/README.md)：短故事的剧本、素材和分镜组合。
- [武松打虎](seedance-storyboard/references/examples/武松打虎/README.md)：动作场景和连续动作分镜。
- [林冲·风雪山神庙](seedance-storyboard/references/examples/林冲-风雪山神庙/README.md)：多场景、对白、动作和资产配合案例。
...
```powershell
git clone https://github.com/oijhl852/seedance-storyboard.git
```

然后在 Reasonix、Codex 或其他支持 Agent Skills 的工具中，只安装仓库里的 `seedance-storyboard` 目录。
...
当前 `v3.0.0` 已经可以投入实际使用。后续优化将以真实使用反馈为依据，重点方向包括：
